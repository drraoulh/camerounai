...
- Si les faits listent des phrases locales, copie-les telles quelles. N’invente jamais un mot dans une langue camerounaise.
...
- If the facts list local phrases, copy them exactly. Never invent a word in a Cameroonian language.
...
export async function POST(req: Request) {
  ...
  const stay = answerStayPhrase(message, locale);
  if (stay) {
    return NextResponse.json({
      answer: stay.answer,
      sourceIds: [],
      tripPlan: null,
      speak: stay.speak ?? null,
      enriched: false,
    });
  }

  const catalog = await fetchDestinations();
  const { answer, sourceIds, tripPlan, phrase, speak } = buildRagAnswer(
    message,
    locale,
    catalog,
  );
  ...
  // Keep structured trip plans and stay-phrase booklets as-is (do not let the LLM invent words)
  const enriched =
    tripPlan || phrase
      ? null
